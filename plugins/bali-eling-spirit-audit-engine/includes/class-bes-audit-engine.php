...
final class BES_Audit_Engine {
    ...
    public function assets( string $hook ): void {
        ...
        wp_localize_script( 'bes-audit-engine', 'BESAuditEngine', array(
            ...
            'labels' => array(
                ...
                'content' => 'Mapping pages and content',
    ...
    public function screen(): void {
        ...
        <div class="wrap bes-audit-wrap"><div class="bes-audit-card">
            ...
            <label class="bes-option"><input id="bes-include-code" type="checkbox" checked> <strong>Include full page content + WPCodeBox snippet code</strong><span>Recommended for migration. Secret-like database columns are omitted automatically.</span></label>
    ...
    public function run_phase(): void {
        ...
        try {
            ...
            if ( 'navigation' === $phase ) { $this->write( "$dir/03-navigation.json", $this->navigation() ); }
    ...
    private function content( bool $full ): array {
        ...
        foreach ( get_post_types( array( 'public' => true ), 'objects' ) as $type => $object ) {
            ...
            $posts = get_posts( array( 'post_type' => $type, 'post_status' => array( 'publish','draft','private','pending','future' ), 'posts_per_page' => -1, 'orderby' => array( 'menu_order' => 'ASC', 'ID' => 'ASC' ), 'suppress_filters' => false ) );
            foreach ( $posts as $post ) {
                $body = (string) $post->post_content;
                $row = array(
                    'id' => (int) $post->ID, 'post_type' => $type, 'post_type_label' => $object->labels->singular_name ?? $type,
        ...
        return array( 'include_full_content' => $full, 'items' => $items );
    }
    ...
    private function navigation(): array {
        ...
        foreach ( wp_get_nav_menus() as $menu ) {
            ...
            foreach ( (array) wp_get_nav_menu_items( $menu->term_id, array( 'post_status' => 'any' ) ) as $item ) {
                ...
                $rows[] = array( 'id' => (int) $item->ID, 'title' => $item->title, 'url' => $item->url, 'parent_menu_id' => (int) $item->menu_item_parent, 'type' => $item->type, 'object' => $item->object, 'object_id' => (int) $item->object_id, 'linked_post_type' => $post ? $post->post_type : null, 'linked_post_title' => $post ? get_the_title( $post ) : null, 'linked_post_slug' => $post ? $post->post_name : null, 'linked_shortcodes' => $post ? $this->extract_shortcodes( (string) $post->post_content ) : array(), 'classes' => array_values( array_filter( (array) $item->classes ) ) );
            }
            $locations = array(); foreach ( get_nav_menu_locations() as $loc => $id ) { if ( (int) $id === (int) $menu->term_id ) { $locations[] = $loc; } }
            $menus[] = array( 'term_id' => (int) $menu->term_id, 'name' => $menu->name, 'slug' => $menu->slug, 'locations' => $locations, 'items' => $rows );
        }
        return array( 'registered_locations' => get_registered_nav_menus(), 'menus' => $menus );
    }
    ...
    private function wpcodebox( bool $full ): array {
        ...
        foreach ( array( 'snippets' => $wpdb->prefix . 'wpcb_snippets', 'folders' => $wpdb->prefix . 'wpcb_folders' ) as $key => $table ) {
            ...
            $safe = array(); foreach ( (array) $rows as $row ) { $safe[] = $this->safe_row( $row, $full ); }
    ...
    private function relationships( string $dir ): array {
        ...
        arsort( $usage );
        return array( 'menu_to_content' => $rows, 'summary' => array( 'content_items' => count( $by_id ), 'menu_relationships' => count( $rows ), 'shortcode_usage_in_menu_targets' => $usage, 'published_pages_not_in_any_menu' => $orphans ) );
    }

    private function finalize( string $id, string $dir, bool $full ): array {
        $files = glob( "$dir/*.json" ); sort( $files );
        $manifest = array( 'engine' => 'Bali Eling Spirit Audit Engine', 'engine_version' => BES_AUDIT_VERSION, 'generated_at_utc' => gmdate( 'c' ), 'include_code' => $full, 'site' => home_url( '/' ), 'files' => array_map( 'basename', $files ), 'notes' => array( 'No database credentials, auth salts, user passwords, WooCommerce orders/customers or session data are intentionally exported.', 'WPCodeBox tables are discovered using the active WordPress table prefix.', 'Password/token/secret/license/key-like WPCodeBox columns are omitted.' ) );
        $this->write( "$dir/00-manifest.json", $manifest ); $files = glob( "$dir/*.json" ); sort( $files );
        if ( class_exists( 'ZipArchive' ) ) { $zip = new ZipArchive(); if ( true !== $zip->open( "$dir/bundle.zip", ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) { throw new RuntimeException( 'Could not create ZIP.' ); } foreach ( $files as $file ) { $zip->addFile( $file, basename( $file ) ); } $zip->close(); $filename = 'bali-eling-spirit-audit.zip'; }
        else { $bundle = array(); foreach ( $files as $file ) { $bundle[ basename( $file ) ] = $this->read( $file ); } $this->write( "$dir/bundle.json", $bundle ); $filename = 'bali-eling-spirit-audit.json'; }
    ...
    private function post_types(): array {
        $out = array(); foreach ( get_post_types( array( 'public' => true ), 'objects' ) as $name => $o ) { $c = wp_count_posts( $name ); $out[] = array( 'name' => $name, 'label' => $o->labels->name ?? $name, 'rest_base' => $o->rest_base, 'has_archive' => $o->has_archive, 'rewrite' => $o->rewrite, 'published' => isset( $c->publish ) ? (int) $c->publish : 0 ); } return $out;
    }
    ...
    private function safe_row( array $row, bool $full ): array { $safe = array(); foreach ( $row as $col => $value ) { $lc = strtolower( $col ); if ( preg_match( '/(^|_)(password|passwd|pwd|secret|token|license|licence|api_key|apikey|private_key|access_key|auth_key)($|_)/', $lc ) ) { $safe[ $col ] = '[omitted-sensitive-column]'; continue; } $code = preg_match( '/(^|_)(code|content|snippet|css|scss|less|javascript|js|html|php|json)($|_)/', $lc ); if ( ! $full && $code && is_string( $value ) ) { $safe[ $col ] = array( 'omitted' => true, 'length' => strlen( $value ), 'sha256' => hash( 'sha256', $value ) ); } else { $safe[ $col ] = $value; } } return $safe; }
    private function dir( string $id ): string { return trailingslashit( get_temp_dir() ) . 'bes-audit-engine/' . $id; }
    private function ensure_dir( string $id ): string { $root = trailingslashit( get_temp_dir() ) . 'bes-audit-engine'; $dir = $this->dir( $id ); if ( ! is_dir( $root ) && ! wp_mkdir_p( $root ) ) { throw new RuntimeException( 'Cannot create temp directory.' ); } if ( ! is_dir( $dir ) && ! wp_mkdir_p( $dir ) ) { throw new RuntimeException( 'Cannot create audit directory.' ); } return $dir; }
    private function write( string $file, array $data ): void { $json = wp_json_encode( $data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ); if ( false === $json || false === file_put_contents( $file, $json ) ) { throw new RuntimeException( 'Cannot write audit data.' ); } }
